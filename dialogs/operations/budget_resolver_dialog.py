...
class BudgetResolverDialog(ComponentDialog):
    ...
    async def initial_step(
        ...
        logger.info("Bot --> : What is your budget for this purchase?")
    ...
    async def budget_prompt_validator(prompt_context: PromptValidatorContext) -> bool:
        ...
        if prompt_context.recognized.succeeded:
            ...
            logger.info(f"user --> budget = {budget}")
            if  float(budget) > 0:
                return True
            else:
                #properties = {"custom_dimensions": {"function": 'summary_step', 'error format or value of budget entred by the user': budget}}
                print("user answer Bad value 0 for budget")
                logger.error("user answer Bad value 0 for budget")
                logger.error("Bad answer!")
        else:
            print("user answer Bad format or value of budget")
            logger.error("user answer Bad format or value of budget")
            logger.error("Bad answer!")
        return False
